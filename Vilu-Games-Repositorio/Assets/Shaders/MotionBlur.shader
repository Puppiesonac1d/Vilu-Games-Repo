...
Shader "Unlit/MotionBlur"
{
    Properties
	{
		...
		_intensity ("Blurring Intensity", Range(0, 1)) = 0
		_move ("Camera Movement", Range(0, 1)) = 0
	}
	Subshader
	{
		Pass
		{
			...
			float4 pixel_shader (float4 vertex:SV_POSITION):COLOR
			{
				...
				uv.y = 1.0 - uv.y;
				uv.x+=cos(uv.y*2.0+_Time.g)*0.05 * _move; // Movimiento eje X
				uv.y+=sin(uv.x*2.0+_Time.g)*0.05 * _move; // Movimiento eje Y
				float offset = sin(_Time.g *0.5) * 0.01 * _intensity; // BLURR
